...
namespace Unity.Core
{
	public static class EnumUtility
	{
		...
		public static IReadOnlyList<T> GetValues<T>()
			where T : struct, IComparable, IConvertible, IFormattable
		{
            var found = s_ValueCache.GetValueOr(typeof(T)) ?? (s_ValueCache[typeof(T)] = Enum.GetValues(typeof(T)));
            return (IReadOnlyList<T>)found;
		}

		public static T TryParseNoCase<T>(string enumName, T defaultValue = default(T))
			where T : struct, IComparable, IConvertible, IFormattable
		{
			T value;
			return Enum.TryParse(enumName, true, out value) ? value : defaultValue;
		}

		public static T TryParse<T>(string enumName, T defaultValue = default(T))
			where T : struct, IComparable, IConvertible, IFormattable
		{
			T value;
			return Enum.TryParse(enumName, false, out value) ? value : defaultValue;
		}
		...
		{
			T value;
			return Enum.TryParse(enumName, true, out value) ? (T?)value : null;
		}
		...
		{
			T value;
			return Enum.TryParse(enumName, false, out value) ? (T?)value : null;
		}

		static IReadOnlyList<string> GetNameCache(Type enumType)
		{
			return s_NameCache.GetOrAdd(enumType, Enum.GetNames);
		}
